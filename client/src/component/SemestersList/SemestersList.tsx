...
const SemestersList = () => {
  ...
  const [major, setMajor] = useState<{ data: SemesterData, isCatalogOpen: boolean }[]>([]);

  const handleClick = (data: SemesterData[]) => {
    const sortedSemesters = data.sort((a, b) => Number(a.semester.value) - Number(b.semester.value));
    const dataToSet = sortedSemesters.map(el => ({ data: el, isCatalogOpen: false }))
    setMajor(dataToSet)
  }

  const toggleCatalog = (courseName: string, open: boolean) => {
    const elementIndex = major.findIndex(el => el.data.courseName.value === courseName);
    ...
    <FacultiesContext.Consumer>
      {(data) => {
        ...
        if (isOpenCatalog) {
          return major.map(el => (
            <div>
              {el.isCatalogOpen && <CourseCatalog data={el.data} onBack={() => toggleCatalog(el.data.courseName.value as string, false)} />}
            </div>
          ))
        }
              ...
              {major.map(el => (
                <Accordion className="test-class">
                  <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <Typography>Semester: {el.data.semester.value}</Typography>
                  </AccordionSummary>
                  <AccordionDetails>
                    <Button variant="contained" onClick={() => toggleCatalog(el.data.courseName.value as string, true)}>Open Catalog</Button>
